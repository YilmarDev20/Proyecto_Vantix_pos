...
export const Modal = ({ isOpen, onClose, title, children, maxWidth = 'md' }: ModalProps) => {
  ...
  return (
    // ✅ ADAPTACIÓN: Fondo oscuro semitransparente con backdrop-blur
    <div className="fixed inset-0 z-50 flex items-center justify-center p-0 md:p-4 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm transition-colors">
      <div 
        // ✅ ADAPTACIÓN: Fondo del modal y sombras para modo oscuro
        className={`bg-white dark:bg-slate-900 w-full h-full md:h-auto rounded-none md:rounded-2xl shadow-xl dark:shadow-slate-950 ${maxWidthClasses[maxWidth]} md:max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-transparent dark:border-slate-800 transition-colors`}
      >
        <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-800 shrink-0 transition-colors">
          {/* ✅ ADAPTACIÓN: Título con color claro para modo oscuro */}
          ...
          <button 
            type="button"
            onClick={onClose} 
            // ✅ ADAPTACIÓN: Botón de cerrar con efecto hover oscuro
        ...
        <div className="p-4 md:p-5 overflow-y-auto flex-1 dark:text-slate-300">
          {children}
        </div>
      </div>
    </div>
